...
def analyze_all(data , n, probL=[0.5]):
    ...
    for p in probL:
        ...
        if data == 'bzip':
            configs = {
                "ReverseLPA": f"{folder}/ILP_greedy_{data}.txt",
                "ILP-PA1": f"{folder}/ILP_{data}_order0.txt",
                "ILP-PA2": f"{folder}/ILP_{data}_order1.txt",
            }
            ...
            configs = {
                "ReverseLPA": f"{folder}/ILP_greedy_{data}.txt",
                "ILP-PA1": f"{folder}/ILP_{data}_order0.txt",
                "ILP-PA2": f"{folder}/ILP_{data}_order3.txt",
            }
...
def generate_final_summary(df, data):
    ...
    for idx, row in summary.iterrows():
        cell_text.append([f"{row['LogLKL']}", f"{row['KendallTau']}", f"{row['KernelSim']}"])

    table = plt.table(
        cellText=cell_text,
        rowLabels=summary.index.tolist(),
        colLabels=["Log-likelihood", "Kendall's Tau", "Kernell Similarity"],
        cellLoc='center',
        loc='center',
        colColours=['#f0f0f0'] * 3
    )
    ...
    table.scale(1, 1.5)
    plt.tight_layout()
    plt.savefig(f"data/{data}/table_{data}.png", bbox_inches='tight', dpi=300)
    plt.close()
...
if __name__ == "__main__":
    ...
    for data_inner_list in data_list:
        ...
        generate_final_summary(df, data_inner_list[0] )
