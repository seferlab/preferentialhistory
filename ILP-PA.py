...
def run_ILP(extantFNAME,outputFNAME, p, timeLimit ,numSolutions, poolMode, focus, numThreads, memusage):
    ...
    for i in range(n):
        neighbours = d[i + 1]
        m.addConstr(y[i, 1, 1] == quicksum(x[e - 1, 1] for e in neighbours), name="y+f'{i}_1_1' == sum (x+f'{e}_1' for e in neighbours)")
    ...
    for t in range(time - 1):
        for k in range(t + 1):
            for i in range(n):
                neighbours = d[i + 1]
                if k < degree:
                    m.addConstr(y[i, k, t + 1] <= 2 - (y[i, k, t] + quicksum(x[e - 1, t] for e in neighbours)), name="y+f'{i}_{k}_{t+1}' <= 2 - (y+f'{i}_{k}_{t}' + sum (x+f'{e}_{t}' for e in neighbours))")
                    m.addConstr(y[i, k, t + 1] >= y[i, k, t] - quicksum(x[e - 1, t] for e in neighbours), name="y+f'{i}_{k}_{t+1}' >= y+f'{i}_{k}_{t}' - sum (x+f'{e}_{t}' for e in neighbours)")
                    m.addConstr(y[i, k, t + 1] <= y[i, k, t] + quicksum(x[e - 1, t] for e in neighbours), name="y+f'{i}_{k}_{t+1}' <= y+f'{i}_{k}_{t}' + sum (x+f'{e}_{t}' for e in neighbours)")
                    if k > 0:
                        m.addConstr(y[i, k, t + 1] <= y[i, k, t] + y[i, k - 1, t], name="y+f'{i}_{k}_{t+1}' <= y+f'{i}_{k}_{t}' + y+f'{i}_{k-1}_{t}")
                        m.addConstr(y[i, k, t + 1] >= y[i, k - 1, t] + quicksum(x[e - 1, t] - 1 for e in neighbours), name="y+f'{i}_{k}_{t+1}' >= y+f'{i}_{k-1}_{t}' + sum (x+f'{e}_{t}' - 1 for e in neighbours)")
    ...
    m.optimize()
    if m.status == GRB.Status.OPTIMAL or m.status == GRB.TIME_LIMIT:
  
       
        nSolutions = m.SolCount
#       print('#Solutions: %s'% str(nSolutions))        
#
#        with open(f"ILP_{file}_p={p}_solution.txt", "w") as f:
#            f.write("objective: ")
#            edge_history = calculate_edge_history(file, m)
#            edges = list(edge_history.items())
# 
#           edges_str = "|".join([f"{u}-{v}\n" for u, v in edges])
#            f.write(f"{m.ObjVal} \n")
#            f.write(f"Edge History:\n|{edges_str}\n")
        ...
        for e in range(nSolutions): 
            ...
            with open(f"ILP_{outputFNAME}_p={p}_solution{e}.txt", "w") as f:
                f.write('Obj: %g\n' % obj_expr.getValue())                
                for v in m.getVars():
                    f.write('%s %g\n' % (v.varName, v.xn))
            
        
            # print('Obj: %g' % obj_expr.getValue())
    else:
...
def calculate_edge_history(extantFNAME, m):
    edge_history = {}
    extant_edges, extant = read_input(extantFNAME)
    E = list(extant.edges())
    rgx = r"\[(\d+),(\d+)\]"
    ...
    for v in m.getVars():
        if v.VarName[0] == 'x':
            if v.Xn == 1.0:
                result = re.search(rgx, v.VarName)
                if result is not None:
                    e = int(result.group(1))
                    t = int(result.group(2))
                    edge_history[t] = E[e]
    edge_history = dict(sorted(edge_history.items()))
    return edge_history
