...
def run_ILP(extantFNAME,outputFNAME, p, timeLimit ,numSolutions, poolMode, focus, numThreads, memusage):
    ...
    if len(E) == 1:
        with open(f"{outputFNAME}_order{0}_time_{timeLimit}.txt", "w") as f:               
    ...
    m.Params.TimeLimit = timeLimit*3600
    ...
    if m.status in (GRB.Status.OPTIMAL, GRB.Status.TIME_LIMIT):
        write_selected_edges(m, E, outputFNAME, timeLimit)
    else:
...
def write_selected_edges(m, candidate_edges, outputFNAME, timeLimit):
